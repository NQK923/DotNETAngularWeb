...
{
    public static void InitBasicInfoAccountApi(this IEndpointRouteBuilder endpointRouteBuilder,
    ...
    public static void MapPutChangeInfoMationAccountByID(this IEndpointRouteBuilder endpointRouteBuilder,
        IConfiguration configuration)
    {
        endpointRouteBuilder.MapPut("/infoAccount/ChangeInfoMationAccountByID",
            async (HttpRequest request, UserServiceDBContext dBContext) =>
            {
                ...
                var infoAccount = await dBContext.infoAccounts.FirstOrDefaultAsync(info =>
                    info.id_account == changeInformationRequest.id_account);
                if (infoAccount == null) return Results.BadRequest("Lỗi MapPutChangeInfoMationAccountByID");
                if (infoAccount.email == null && infoAccount.name.Equals(changeInformationRequest.name) && file == null)
                    return Results.Ok(false);
                if (infoAccount.email != null && infoAccount.email.Equals(changeInformationRequest.email) &&
                    infoAccount.name.Equals(changeInformationRequest.name) && file == null) return Results.Ok(false);
                ...
                if (!string.IsNullOrEmpty(changeInformationRequest.email))
                {
                    ...
                    var response = await httpClient.GetAsync(
                        $"https://emailvalidation.abstractapi.com/v1/?api_key=a2d97dea13244bc0bb8ec58a0f5080c7&email={changeInformationRequest.email}");
                    var content = await response.Content.ReadAsStringAsync();
                    var jsonResponse = JObject.Parse(content);
                    var isSmtpValid = jsonResponse["is_smtp_valid"]["value"].Value<bool>();
                    if (isSmtpValid)
                        infoAccount.email = changeInformationRequest.email;
                    else
                        return Results.Ok("Email không hợp lệ");
                }

                if (!string.IsNullOrEmpty(changeInformationRequest.name))
                    infoAccount.name = changeInformationRequest.name;
                if (file != null)
                {
                    ...
                    if (infoAccount.cover_img.Contains("New"))
                        blobName = $"IA{infoAccount.id_account}.jpg";
                    else
                        blobName = $"NewIA{infoAccount.id_account}.jpg";
                    ...
                    var img = blobClient.Uri.ToString();
    ...
    {
        endpointRouteBuilder.MapGet("/infoAccount/GetInfoMationAccountByID",
            async (int idAccount, UserServiceDBContext dBContext) =>
            {
                var infoAccount =
                    await dBContext.infoAccounts.FirstOrDefaultAsync(info => info.id_account == idAccount);
                if (infoAccount == null) return Results.BadRequest();
                return Results.Ok(infoAccount);
            });
    }
    ...
    public static void MapPostValidEmail(this IEndpointRouteBuilder endpointRouteBuilder)
    {
        endpointRouteBuilder.MapPost("/infoAccount/CheckEmailValid", async (string email) =>
        {
            using var httpClient = new HttpClient();
            var response = await httpClient.GetAsync(
                $"https://emailvalidation.abstractapi.com/v1/?api_key=a2d97dea13244bc0bb8ec58a0f5080c7&email={email}");
            var content = await response.Content.ReadAsStringAsync();
            var jsonResponse = JObject.Parse(content);
            var isSmtpValid = jsonResponse["is_smtp_valid"]["value"].Value<bool>();
    ...
    public static void MapPostAddInfomation(this IEndpointRouteBuilder endpointRouteBuilder,
        IConfiguration configuration)
    {
        endpointRouteBuilder.MapPost("/infoAccount/AddInfomation",
            async (InfoMationRegisterRequest infoMationRegisterRequest, UserServiceDBContext dBContext) =>
            {
                var img = "";
                if (!infoMationRegisterRequest.img.Equals(
                        "https://dotnetmangaimg.blob.core.windows.net/avatars/defaulImage.png"))
                {
                    var httpClient = new HttpClient();
                    var imageBytes = await httpClient.GetByteArrayAsync(infoMationRegisterRequest.img);
