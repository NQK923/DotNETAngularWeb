namespace Account
{
    ...
    using System.Text;
    using static Account.Model.Requests;
    ...
    namespace WebApplication2
    {
        public static class AccountAPI
        {
            ...
            {
                endpointRouteBuilder.MapPut("/account/changePasswordAccountByID", async (ChangePasswordRequest changePasswordRequest, AccountDbContext dBContext) =>
                {
                    ...
                    if (!account.password.Equals(changePasswordRequest.oldPassword)) return Results.BadRequest("Mật khẩu hiện tại không đúng");
            ...
            {
                endpointRouteBuilder.MapPost("/account/checkOldPasswordAccountByID", async (CheckOldPasswordRequest checkOldPasswordRequest, AccountDbContext dBContext) =>
                {
            ...
            public static void MapGetListAccount(this IEndpointRouteBuilder endpointRouteBuilder)
            {
                ...
                {
                    List<ModelAccount> list = await dBContext.accounts.ToListAsync();
            ...
            {
                endpointRouteBuilder.MapGet("/account/getAccountByID", async (int idAccount, AccountDbContext dBContext) =>
                {
            ...
            {
                endpointRouteBuilder.MapPost("/account/register", async (LoginRegisterRequest loginRegisterRequest, AccountDbContext dBContext) =>
                {
                    // Kiểm tra tài khoản có tồn tại

                    var newUsername = await dBContext.accounts.FirstOrDefaultAsync(account => account.username == loginRegisterRequest.username);

                    if (newUsername != null)
                    {
                        return Results.BadRequest("Tài khoản đã tồn tại");
                    }
            ...
            {
                endpointRouteBuilder.MapPost("/account/externalAccountRegister", async (string username, AccountDbContext dBContext) =>
                {
                    var newAccount = new ModelAccount
                    {
                        username = username,
            ...
            {
                endpointRouteBuilder.MapPost("/account/externalLoginIsExist", async (string username, AccountDbContext dBContext) =>
                {
                    var account = await dBContext.accounts.FirstOrDefaultAsync(acc => acc.username == username);
                    if (account == null) return Results.BadRequest("Tài khoản không tồn tại");
                    if (!account.status) return Results.BadRequest(account.banDate);
            ...
            {
                endpointRouteBuilder.MapPut("/account/changeStatusAccountByID", async (ChangeStatusRequest changeStatusRequest, AccountDbContext dBContext) =>
                {
            ...
            {
                endpointRouteBuilder.MapPut("/acccount/changeBanDateAccountByID", async (ChangeBanDateRequest changeBanDateRequest, AccountDbContext dBContext) =>
                {
        ...
        new Claim("idAccount", idAccount.ToString()) // Thêm claim cho idAccount
                ...
                var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("yourSuperSecretKey12345678901234567890")); // Đảm bảo khóa có độ dài ít nhất 32 bytes
                var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

                var token = new JwtSecurityToken(
                    issuer: "https://localhost:4200/", // Đặt giá trị issuer
                    audience: "https://localhost:4200/", // Đặt giá trị audience
                    claims: claims,
